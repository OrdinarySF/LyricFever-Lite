...
@MainActor class viewModel: ObservableObject {
    ...
    @Published var currentLyricSource: String = "" // Track where lyrics came from
    ...
    init() {
        ...
        LRCLIBUserAgentConfig.httpAdditionalHeaders = ["User-Agent": "Lyric Fever v3.0 ()"]
...
//        https://musicbrainz.org/ws/2/release/?query=artist:charli%20xcx%20AND%20album:super%20ultra&fmt=json
    ...
    func generateAccessToken() async throws {
        ...
        if accessToken == nil || (accessToken!.accessTokenExpirationTimestampMs <= Date().timeIntervalSince1970*1000) {
            ...
            guard !cookie.isEmpty else {
                print("No sp_dc cookie available")
                // Don't reset onboarding - app works without Spotify auth now
            ...
            repeat {
                ...
                do {
                    ...
                    if let totp = viewModel.TOTPGenerator.generate(serverTimeSeconds: serverTime), let url = URL(string: "https://open.spotify.com/get_access_token?reason=transport&productType=web-player&totp=\(totp)&totpServer=\(Int(Date().timeIntervalSince1970))&totpVer=5&sTime=\(serverTime)&cTime=\(serverTime)") {
                        ...
                        let (accessTokenData, response) = try await fakeSpotifyUserAgentSession.data(for: request)
                        ...
                        if let httpResponse = response as? HTTPURLResponse {
                            ...
                            if httpResponse.statusCode == 401 {
                                print("401 Unauthorized - Cookie is invalid or expired")
                                // Don't reset onboarding - app works without Spotify auth now
                                cookie = ""
                                throw AccessTokenError.toomanytries
                            }
                        ...
                        print(String(decoding: accessTokenData, as: UTF8.self))
                        ...
                        do {
                            ...
                            do {
                                let errorWrap = try JSONDecoder().decode(ErrorWrapper.self, from: accessTokenData)
                                if errorWrap.error.code == 401 {
                                    UserDefaults().set(false, forKey: "hasOnboarded")
                                    cookie = ""
                                }
                                print("Error from Spotify: \(errorWrap.error)")
                            } catch {
                                // Check if it's a plain text error
                                let errorString = String(decoding: accessTokenData, as: UTF8.self)
                                if errorString.lowercased().contains("invalid") || errorString.lowercased().contains("expired") {
                                    UserDefaults().set(false, forKey: "hasOnboarded")
                                    cookie = ""
                                }
                            }
                    ...
                    print("Error during token generation: \(error)")
                }
            ...
            } while (accessToken?.accessToken.range(of: "[-_]", options: .regularExpression) == nil || !fetchHomeResult) && repeatCount < 3
        }
    }
    ...
    func fetchNetworkLyrics(for trackID: String, _ trackName: String, _ spotifyOrAppleMusic: Bool) async throws -> [LyricLine] {
        ...
        // Always use LRCLIB -> QQ Music -> NetEase flow
        print("Fetching lyrics for: \(trackName)")
        ...
        if !lrc.isEmpty {
            ...
            currentLyricSource = "LRCLIB"
            return lrc
        }

        // If LRCLIB fails, try QQ Music
        print("LRCLIB failed, trying QQ Music...")
        let qqMusic = (try? await fetchQQMusicLyrics(trackName: trackName, spotifyOrAppleMusic: spotifyOrAppleMusic, trackID: trackID)) ?? []
        if !qqMusic.isEmpty {
            print("Found lyrics from QQ Music")
            currentLyricSource = "QQ Music"
            return qqMusic
        }

        // If QQ Music fails, try NetEase
        print("QQ Music failed, trying NetEase...")
        let netease = (try? await fetchNetEaseLyrics(trackName: trackName, spotifyOrAppleMusic: spotifyOrAppleMusic, trackID: trackID)) ?? []
        if !netease.isEmpty {
            print("Found lyrics from NetEase")
            currentLyricSource = "NetEase"
            return netease
        }
...
//        if let artist, let album, let url = URL(string: "https://lrclib.net/api/get?artist_name=\(artist)&track_name=\(trackName)&album_name=\(album)&duration=\(spotifyOrAppleMusic ? intDuration : intDuration / 1000)") {
        ...
        if let currentlyPlayingArtist, let album, let url = URL(string: "https://neteasecloudmusicapi-ten-wine.vercel.app/search?keywords=\(trackName.replacingOccurrences(of: "&", with: "%26")) \(currentlyPlayingArtist.replacingOccurrences(of: "&", with: "%26"))&limit=1") {
            ...
            // For Chinese songs, also check if strings contain each other as substrings
            let trackNameMatch = trackName.distance(between: neteaseResult.name) > 0.75 ||
                                trackName.contains(neteaseResult.name) ||
                                neteaseResult.name.contains(trackName)

            let artistMatch = currentlyPlayingArtist.distance(between: neteaseArtist.name) > 0.75 ||
                             currentlyPlayingArtist.contains(neteaseArtist.name) ||
                             neteaseArtist.name.contains(currentlyPlayingArtist)

            let albumMatch = album.distance(between: neteaseResult.album.name) > 0.75 ||
                            album.contains(neteaseResult.album.name) ||
                            neteaseResult.album.name.contains(album)

            let conditions = [trackNameMatch, artistMatch, albumMatch]
            ...
            // For Chinese songs, be more lenient with similarity matching
            // Check if the song contains Chinese characters
            let containsChinese = trackName.contains { character in
            ...
            // If it's a Chinese song, only require 1 condition to pass
            // Otherwise, require 2 conditions as before
            let requiredConditions = containsChinese ? 1 : 2

            if trueCount < requiredConditions {
                print("similarity conditions passed for NetEase: \(trueCount) is less than \(requiredConditions), therefore failing this NetEase search.")
                currentLyricSource = ""
                return []
            }
...
//        if let artist, let album, let url = URL(string: "https://lrclib.net/api/get?artist_name=\(artist)&track_name=\(trackName)&album_name=\(album)&duration=\(spotifyOrAppleMusic ? intDuration : intDuration / 1000)") {
        if let trackName, let artist = artist, let album = album, let url = URL(string: "https://lrclib.net/api/get?artist_name=\(artist)&track_name=\(trackName)&album_name=\(album)") {
            ...
            let responseString = String(data: urlResponseAndData.0, encoding: .utf8) ?? "Unable to decode response"
            print("LRCLIB response (\(urlResponseAndData.0.count) bytes): \(responseString)")

            // Check if response is too small to be valid lyrics
            if urlResponseAndData.0.count < 100 {
                print("LRCLIB returned empty or minimal response, likely no lyrics found")
                currentLyricSource = ""
                return []
            }
...
//            }
    ...
    func fetchQQMusicLyrics(trackName: String, spotifyOrAppleMusic: Bool, trackID: String) async throws -> [LyricLine] {
        let album = spotifyOrAppleMusic ? appleMusicScript?.currentTrack?.album : spotifyScript?.currentTrack?.album

        // Search for the song on QQ Music
        guard let song = try await QQMusicAPI.shared.searchSong(track: trackName, artist: currentlyPlayingArtist, album: album) else {
            print("QQ Music: No matching song found")
            currentLyricSource = ""
            return []
        }

        print("QQ Music: Found song '\(song.songname)' by \(song.singer.map { $0.name }.joined(separator: ", "))")

        // Get the lyrics
        let (lyrics, source) = try await QQMusicAPI.shared.fetchLyrics(songmid: song.songmid)

        if lyrics.isEmpty {
            print("QQ Music: No lyrics available for this song")
            currentLyricSource = ""
            return []
        }

        print("QQ Music: Successfully fetched \(lyrics.count) lines of lyrics")

        // Save to CoreData
        let songObject = SongObject(from: lyrics, with: coreDataContainer.viewContext, trackID: trackID, trackName: trackName, duration: decoder.userInfo[CodingUserInfoKey.duration] as! TimeInterval)
        saveCoreData()
        amplitude.track(eventType: "QQ Music Fetch")
        currentLyricSource = source

        // Save album artwork color for karaoke mode
        if spotifyOrAppleMusic {
            if let artwork = (appleMusicScript?.currentTrack?.artworks?().firstObject as? MusicArtwork)?.data {
                SpotifyColorData(trackID: trackID, context: coreDataContainer.viewContext, background: artwork.findWhiteTextLegibleMostSaturatedDominantColor())
            } else if let albumName = appleMusicScript?.currentTrack?.album, let artistName = currentlyPlayingArtist, let mbid = await findMbid(albumName: albumName, artistName: artistName), let artworkUrl = mbidAlbumArt(mbid), let imageData = try? await URLSession.shared.data(from: artworkUrl), let image = NSImage(data: imageData.0) {
                SpotifyColorData(trackID: trackID, context: coreDataContainer.viewContext, background: image.findWhiteTextLegibleMostSaturatedDominantColor())
            }
        } else {
            if let artworkUrlString = spotifyScript?.currentTrack?.artworkUrl, let artworkUrl = URL(string: artworkUrlString), let imageData = try? await URLSession.shared.data(from: artworkUrl), let image = NSImage(data: imageData.0) {
                SpotifyColorData(trackID: trackID, context: coreDataContainer.viewContext, background: image.findWhiteTextLegibleMostSaturatedDominantColor())
            } else if let albumName = spotifyScript?.currentTrack?.album, let artistName = currentlyPlayingArtist, let mbid = await findMbid(albumName: albumName, artistName: artistName), let artworkUrl = mbidAlbumArt(mbid), let imageData = try? await URLSession.shared.data(from: artworkUrl), let image = NSImage(data: imageData.0) {
                SpotifyColorData(trackID: trackID, context: coreDataContainer.viewContext, background: image.findWhiteTextLegibleMostSaturatedDominantColor())
            }
        }
...
extension viewModel {
    ...
    func appleMusicFetch() async throws {
        // No longer need to convert to Spotify ID
        // Just set currentlyPlaying to a unique identifier for Apple Music songs
        if let persistentID = currentlyPlayingAppleMusicPersistentID {
            self.currentlyPlaying = "apple_music_\(persistentID)"
        }
    }
    ...
    func appleMusicNetworkFetch() async throws {
        // This function is no longer needed since we don't convert to Spotify IDs
        // The name and artist are already set from the Apple Music script
        return
    }
